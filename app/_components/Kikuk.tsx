...
export default function Kikuk({name, open, close, data, className, onClick} : {name: string, open: boolean, close: Function ,data: {index: number, name: string}[], className: string, onClick: React.MouseEventHandler<HTMLButtonElement>}) {
   ...
   useEffect(() => {
      console.log('select open', listboxRef.current);
   ...
   return (
      <>
         ...
         >
            {field.value.name}
            <Arrow className={
               clsx('text-primary w-[18px] h-[18px] absolute right-[12px] top-[10px] transition-transform', open && 'rotate-180')
            } />
         </button>
         {open && 
         createPortal(
